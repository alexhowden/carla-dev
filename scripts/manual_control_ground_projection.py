#!/usr/bin/env python
"""
Manual drive + Segmentation + Inverse Perspective Mapping (IPM) bird's-eye view in CARLA.
Supports both SegFormer (multi-class) and DeepLabV3+ ORFD (binary freespace).

Shows four panels side-by-side:
    [Camera feed] [Segmentation] [BEV segmentation] [Legend]
...
Requires: CARLA running, pip install -r requirements-segmentation.txt pynput

Usage:
    python scripts/manual_control_ground_projection.py
    python scripts/manual_control_ground_projection.py --model training/models/rellis3d_b0_ade_arc
    python scripts/manual_control_ground_projection.py --deeplab
    python scripts/manual_control_ground_projection.py --bev-x 8 --bev-ymin 5 --bev-ymax 30
...
def main():
    parser = argparse.ArgumentParser(
        description="Manual drive (WASD) + Segmentation + Bird's Eye View ground projection in CARLA."
    )
    ...
    parser.add_argument("--max-inference-fps", action="store_true",
    ...
    window_name = "Manual + Ground Projection (BEV)"
    ...
    try:
        while True:
            ...
            if bev_bgr is not None and seg_bgr is not None:
                ...
                cv2.putText(combined, "Speed: %.0f km/h%s" % (speed_kmh, rev_str),
